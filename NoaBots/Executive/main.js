...
moment.locale("tr");
kanal.send(`❃ Utopia'ya Hoşgeldin ${member}
...
client.on("guildMemberAdd", async noa => {
const kanal = noa.guild.channels.cache.find(r => r.id === config.HoşgeldinKanalID);
const tarih = new Date().getTime() - noa.user.createdAt.getTime();
   if (tarih < 1000*360*247 ) {
   noa.roles.add(config.ŞüpheliHesap)
   noa.roles.remove(config.unregisterRoles)
   kanal.send(`${noa} ${noa.id} Adlı kullanıcının hesabı yeni açıldığı için jaile atıldı.`)
   noa.send(`Hesabın 7 günden az bi sürede açıldığı için jaile atıldın. Eğer bir yanlışlık olduğunu düşünüyorsan yanda bulunan yetkililere yazabilirsin.`)}
}); 
...
client.on("guildMemberAdd", member => {
if(member.user.username.includes("") && (member.user.username.includes("")) && (member.user.username.includes("") && (member.user.username.includes("")))){
...
client.on("guildMemberAdd", member => {
const noacik = member.guild.channels.cache.find(r => r.id === config.HoşgeldinKanalID);
if(member.user.username.includes(config.tag)){
member.roles.add(config.Tagges)
noacik.send(`${member} (\`${member.id}\`) Sunucuya Taglı Bir Şekilde Giriş Yaptı!`)}
})

async function yasaklitag() {
  let guild = client.guilds.cache.get(config.guildID);
  let data = await db.get(`yasaklitaglar.${guild.id}`) || []  
guild.members.cache.filter(s => data.some(x => s.user.username.includes(x)) && !s.roles.cache.has("")).forEach(m => m.roles.set(['']))}
setInterval(async() => {
yasaklitag()
}, 60000)
