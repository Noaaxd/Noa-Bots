...
let Noa = message.guild.members.cache.filter(m => m.roles.cache.filter(r => r.id !== message.guild.id).size == 0)
...
.setColor("2F3136")
.setDescription("Sunucuda rolü olmayan \`"+ Noa.size +"\` kişiye \`Kayıtsız\` rolü verildi")
...
.setFooter(config.Footer)
.setColor("2F3136")
.setDescription("Sunucumuzda rolü olmayan \`"+ Noa.size +"\` kişi var.")
