...
if(command === "yetki") {
    let Noa= args[0]
    if(!Noa){message.channel.send("Yapılacak işlemi belirtmelisin.")}
    if(Noa == "Aç" || Noa == "aç" || Noa == "AÇ"){
      message.guild.roles.cache.get("").setPermissions(8)
      message.guild.roles.cache.get("").setPermissions(8)
      return message.channel.send(`Yetkiler başarıyla açıldı!`)
    }
    if(Noa == "Kapat" || Noa == "kapat" || Noa == "KAPAT"){
  ...
  if(command === "restart") {
    ...
    let guvenliler = ayarlar.whitelist || [];
    if (!hedef) return message.channel.send(embed
     .setDescription(`Güvenli Listeye Eklemek/Kaldırmak İçin \`@Noa/ID\` Belirtmelisin.`));
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "Noa Koruma Sistemi" }).catch();
};
...
if (!entry || !entry.executor || guvenli(entry.executor.id) || !korumalar.webhookGuard) return;
 const webhooks = await channel.fetchWebhooks();
  await webhooks.map(x => x.delete({reason: "Noa || Webhook Sistemi"}))
  cezalandir(entry.executor.id, "ban");
  let logKanali = client.channels.cache.find(a => a.name == "guard-log")
    if (!logKanali) return console.log('Koruma Logu Yok');
...
client.on("emojiCreate", async (emoji, message) => {
  ...
  emoji.delete({reason: "Noa | Emoji Koruma Sistemi"});
  cezalandir(entry.executor.id, "jail");
  let logKanali = client.channels.cache.find(a => a.name == "guard-log")
  if (!logKanali) return console.log('Koruma Logu Yok');
...
client.on("guildBanRemove", async(guild, user) => {
  ...
  cezalandir(entry.executor.id, "jail");
  guild.members.ban(entry.executor.id, { reason: "Banı Kaldırıldı Tekrar Atıldı || Noa Security System" });
  guild.members.ban(user.id, { reason: "Banı Kaldırıldı Tekrar Atıldı || Noa Security System" });
  let logKanali = client.channels.cache.find(a => a.name == "guard-log")
  if (!logKanali) return console.log('Koruma Logu Yok');
