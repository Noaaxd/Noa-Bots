...
module.exports = class {
    ...
    async run(menu) {
        ...
        if (menu.values[0] == 'lgbt') {
            if (!member.roles.cache.has(this.client.config.event.lgbt)) {
              await member.roles.add(this.client.config.event.lgbt)
              await member.roles.remove(this.client.config.event.lgbt)
              return menu.reply.send('LGBT rolü üzerinize verildi.', true)
            } else if (member.roles.cache.has(this.client.config.event.lgbt)) {
              await member.roles.remove(this.client.config.event.lgbt)
              return menu.reply.send("LGBT rolü üzerinizden alındı.", true)
            }
        }
